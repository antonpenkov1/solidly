# Spoonlet — App Store Connect metadata
...
`Spoonlet` carries the category term — *starting solids* — and, read as an adverb, the
...
name as a keyword implies endorsement Spoonlet does not have, and that is a rejection risk as well
as being untrue.
...
```
Spoonlet is a feeding tracker that shows its work.
...
word about what to feed or why. Spoonlet does both, and names its sources for all of it.
...
introduction ages they gave you and Spoonlet follows those instead, noting who prescribed them —
...
Spoonlet is not a medical device and does not diagnose. It summarises published guidance so you can
...
```
Spoonlet — дневник кормления, который показывает, откуда он берёт цифры.
...
роста, но ни слова не говорят о том, чем и почему кормить. Spoonlet делает и то, и другое — и на
...
возрасты введения, которые назвал он, — Spoonlet будет следовать им и подписывать, кто их назначил,
...
Spoonlet — не медицинское изделие и не ставит диагнозов. Оно пересказывает опубликованные
...
- Support URL: `https://antonpenkov1.github.io/spoonlet/`
- Privacy Policy URL: `https://antonpenkov1.github.io/spoonlet/privacy.html` (source: `docs/privacy.html`)
...
• Onboarding requires an explicit acknowledgement, recorded with a timestamp, that Spoonlet is not a
